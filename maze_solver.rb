class MazeSolver
  ...
  # ----------------------------------------------------------------------------
  # can everything below this line safely be made private?
  ...
  def choose_next_cell
    # why does @location need the @ here and in #display, but nowhere else?
    # do these lines invalidate the recalculations in update_movement_cost?
    ...
    candidate_locations.each do |pos, cost|
      if candidate_locations[pos] == candidate_locations.values.min
        #this if-statement might slow things down some when multiple mins exist
  ...
  def update_movement_cost(cell)
    # idk if maze1 actually needs this, but it's necessary in some cases.
    # Ugly and hairy, but the alternative is refactoring
    # calculate_movement_costs to handle all this.
    ...
    if new_movement_cost < current_movement_cost
      ...
      calculate_net_costs(cell)       #does .each break if given only one thing?
    end
  end
  ...
  def highlight_completed_path
    ...
    # Also removes • marks denoting positions on the closed list.
  ...
  def write_solution_to_file
    File.open("#{ARGV[0]}_solved.txt", "w") do |f|
      maze.each { |line| f.puts line.join } #adds a blank line at the end. ???
    end
  end
